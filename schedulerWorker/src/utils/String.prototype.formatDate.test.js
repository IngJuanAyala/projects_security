require('./String.prototype.formatDate');
const { expect } = require('chai');
const sinon = require('sinon');
const logger = require('@condor-labs/logger');

const string = '08/16/2018 00:00:00 America/Bogota';
const stringWithOutHour = '08/16/2018';
...
describe(`When String.prototype.fomatDate is called`, () => {
  ...
  describe(`When include Timezone is specified in options`, () => {
    it(`Should return the date in the format passed`, () => {
      expect(string.formatDate({ includeHours: true, includeTimezone: true })).eql(
        `2018-08-16 00:00:00 America/Bogota`
      );
    });
  });
  ...
  describe(`When not include hours and a format  is specified but include TimeZone doesn't in options`, () => {
    it(`Should return the date in the format passed`, () => {
      sinon.stub(logger, 'warn');

      expect(typeof stringWithOutHour.formatDate({ format: 'YYYY-DD-MM', includeHours: false })).eql('undefined');
